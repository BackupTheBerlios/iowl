...
class cOwlManager:
    ...
    def Distribute(self, domObj):
        ...
        try:
            ...
            version, protocol, id, type, iTTL, orgIP, orgPort = self.GetDomInfo(domObj)
        except:
        ...
        if protocol != self.cNetManager.sProtocol:
            ...
            pManager.manager.DebugStr('cOwlManager '+ __version__ +': Warning: Received Dom with different network version for distribution. Skipping...')
            return 'error'
        ...
        if type == 'Ping':
            ...
            if str(OrigOwl.IP) != str(pManager.manager.GetOwnIP()):
                ...
                elRequest = self.GetElementFromDOM(domObj, 'itemset')
                ...
                # pManager.manager.DebugStr('cOwlManager '+ __version__ +': Got request from myself.')
                pass
        ...
        if (str(reqAttributes[self.SourceOwl].IP)    == str(pManager.manager.GetOwnIP())) and \
           (str(reqAttributes[self.SourceOwl].iPort) == str(self.cNetManager.cNetServer.GetListenPort())):
            ...
            if type == 'Pong':
                ...
                elAnswer = self.GetElementFromDOM(domObj, 'rules')
